...
namespace MP
{
	MPfloat MPfloat::operator-(const MPfloat& mpf)const
	{
		...
		for (int i = 0; i < out.m_data.len; i++)
		{
			...
			if (in1 && in2)
			{
				rem = _subborrow_u64(rem, a1[i + e3 - e1], a2[i + e3 - e2], a3 + i);
			}
			else
			{
				if (in1 && !in2)
				{
					rem = _subborrow_u64(rem, a1[i + e3 - e1], 0, a3 + i);
				}
				else
				{
					if (!in1 && in2)
						rem = _subborrow_u64(rem, 0, a2[i + e3 - e2], a3 + i);
					else
						rem = _subborrow_u64(rem, 0, 0, a3 + i);
				}
			}
		}
